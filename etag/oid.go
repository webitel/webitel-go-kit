...
func GetOid(typ EtagType, id string) (oid int64, err error) {
	...
	if err != nil || oid < 1 {
		err = NewBadRequestError(typ, "id.invalid", "node( id:%s ); expected: positive int64", id)
		return
	}
	return
}
...
func GetId(typ EtagType, oid int64) (id string, err error) {
	if oid < 1 {
		err = NewInternalError(typ, "id.invalid", "node( id:%d ); expected: positive int64", oid)
		return
	}
...
func MustId(typ EtagType, id string, err error) string {
	if err == nil && id == "" {
		err = NewInternalError(typ, "id.required", "node( id: ); required")
	}
	if err != nil {
		panic(err)
	}
