	...
	return func(node TNode) error {
		if node.GetId() == "" {
			return NewBadRequestError(
				typ,                         // Pass the ETag type as a parameter
				"id_missing",                // Error key for missing ID
				"%s (id: ); ID is required", // Error message
				nodeOf,                      // Include node type (like "node") in the message
			)
		}
		return nil
	}
