// Package wlog provides a bridge between the [go.uber.org/zap] and [OpenTelemetry].
//
// # Record Conversion
//
...
const coreName = "github.com/webitel/webitel-go-kit/logging/wlog"
...
// OTLPBridgeCore is a [zapcore.Core] that sends logging records to OpenTelemetry.
type OTLPBridgeCore struct {
	provider log.LoggerProvider
	logger   log.Logger
...
var _ zapcore.Core = (*OTLPBridgeCore)(nil)

// NewOTLPBridgeCore creates a new [zapcore.Core] that can be used with [go.uber.org/zap.New].
...
func NewOTLPBridgeCore(name string, opts ...Option) *OTLPBridgeCore {
	cfg := newConfig(opts)
...
func (o *OTLPBridgeCore) Enabled(level zapcore.Level) bool {
	r := log.Record{}
...
func (o *OTLPBridgeCore) With(fields []zapcore.Field) zapcore.Core {
	cloned := o.clone()
	if len(fields) > 0 {
...
func (o *OTLPBridgeCore) clone() *OTLPBridgeCore {
	return &OTLPBridgeCore{
		provider: o.provider,
		opts:     o.opts,
		logger:   o.logger,
...
func (o *OTLPBridgeCore) Sync() error {
	return nil
}

// Check determines whether the supplied Entry should be logged.
// If the entry should be logged, the Core adds itself to the CheckedEntry and returns the result.
func (o *OTLPBridgeCore) Check(ent zapcore.Entry, ce *zapcore.CheckedEntry) *zapcore.CheckedEntry {
	r := log.Record{}
...
func (o *OTLPBridgeCore) Write(ent zapcore.Entry, fields []zapcore.Field) error {
	r := log.Record{}
